...
const FormationTable: React.FC<FormationTableProps> = ({ 
  ...
  const [uniqueDepartements, setUniqueDepartements] = useState<string[]>([]);
  const [uniqueVilles, setUniqueVilles] = useState<string[]>([]);
  
  useEffect(() => {
    // Extract unique departements and villes from formations
    const departements = Array.from(new Set(formations.map(f => f.departement))).sort();
    const villes = Array.from(new Set(formations.map(f => f.ville))).sort();
    setUniqueDepartements(departements);
    setUniqueVilles(villes);
  }, [formations]);
  ...
  const handleDepartementChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    onFilterChange({ ...filters, departement: e.target.value });
  };
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              <th 
                scope="col" 
                className="w-[20%] px-0 py-8 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100 transition"
                onClick={() => handleSortClick('etablissement')}
              >
                <div className="transform -rotate-45 origin-left h-24 flex items-end">
                  <span>Établissement {getSortIcon('etablissement')}</span>
                </div>
              </th>
              <th 
                scope="col" 
                className="w-[50%] px-0 py-8 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100 transition"
              ...
              <th 
                scope="col" 
                className="w-[5%] px-0 py-8 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100 transition"
              ...
              <th 
                scope="col" 
                className="w-[15%] px-0 py-8 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100 transition"
