...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (event.type === "checkout.session.completed") {
      ...
      if (fetchError) {
        console.error("❌ Failed to fetch current tokens:", fetchError);
        throw new Error(`Failed to fetch current tokens: ${fetchError.message}`);
      }

      const currentTokens = currentProfile?.tokens || 0;
      const newTokens = currentTokens + 1;

      // Update tokens with direct value
      ...
      console.log(`✅ Successfully updated tokens for user ${userId} from ${currentTokens} to ${newTokens}`);
    }
